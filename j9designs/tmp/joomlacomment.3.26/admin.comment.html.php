...
class HTML_comments {
    ...
    {
        global $mosConfig_live_site, $mainframe, $mosConfig_absolute_path;
		...
		$component_exist = ($lists['component']=="" || @is_dir("$mosConfig_absolute_path/components/".$lists['component']));

?>
<?php 	if ($adminform) { ?><form action="index2.php" method="post" name="adminForm"><?php 	} ?>

    <table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminheading">
    	<tr>
      		<th width="100%" align="left"><?php echo $lists['title'] ?></th>
      		<td><?php echo $lists['componentlist']; ?>
			</td>
      		<td nowrap="nowrap">Display #</td>
      		<td> <?php echo $pageNav->writeLimitBox(); ?>
      		</td>
      		<td>Search:</td>
      		<td>
...
?>
    ...
    <form action="index2.php" method="post" name="adminForm" id="adminForm">
      ...
      <tr>
        ...
        <td width="80%">
          <?php echo mosHTML::yesnoRadioList('notify', 'class="inputbox"', $row->notify); ?>
        </td>
      </tr>
...
<?php		if (!$onlydisplay) { 
?>
			<a href="index2.php?option=<?php echo $option; ?>&task=settingsedit<?php echo $expertmode ? "expert" : ""; ?>&id=<?php echo $row->id ?>">
...
<?php
	    ...
	    $tabs = new mosTabs(1);
        $tabs->startPane("jos_importpanel");
        
        $tabs->startTab("Mapping", "mapping");
        importMapping($option, $task, $fromtable, $fromcomponent, $component);
        $tabs->endTab();
	
        $tabs->startTab("Preview", "preview");
	...
	{				           		
		...
		$rows->createRow('parameter', 
		'Commented component column', mosHTML::selectList( $lists['columns'], 'componentfield', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['componentfield'] ), 'Select the column which contains the <b>Component name selection</b>.' );
		$rows->createRow('parameter', 
		'--- Id from the column <b style="color:red;">(obligatory)</b>', mosHTML::selectList( $lists['columns'], 'id', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['id'] ), 'Select the column which contains the <b>Comment Id</b>.' );
		$rows->createRow('parameter', 
		'--- Content Id from the column <b style="color:red;">(obligatory)</b>', mosHTML::selectList( $lists['columns'], 'contentid', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['contentid'] ), 'Select the column which contains the <b>Content Item Id</b>' );
		$rows->createRow('parameter', 
		'--- Date from the column', mosHTML::selectList( $lists['columns'], 'date', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['date'] ), 'Select the column which contains the <b>Date of the comment</b>' );
		$rows->createRow('parameter', 
		'--- Name from the column', mosHTML::selectList( $lists['columns'], 'name', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['name'] ), 'Select the column which contains the <b>Name</b> of the comment writer' );
		$rows->createRow('parameter', 
		'--- Userid from the column', mosHTML::selectList( $lists['columns'], 'userid', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['userid'] ), 'Select the column which contains the <b>Userid</b> of the comment writer' );
		$rows->createRow('parameter', 
		'--- IP from the column', mosHTML::selectList( $lists['columns'], 'ip', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['ip'] ), 'Select the column which contains the <b>IP</b> of the comment writer' );
		$rows->createRow('parameter', 
		'--- Email from the column', mosHTML::selectList( $lists['columns'], 'email', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['email'] ), 'Select the column which contains the <b>Email</b> of the comment writer' );
		$rows->createRow('parameter', 
		'--- Notify parameter from the column', mosHTML::selectList( $lists['columns'], 'notify', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['notify'] ), 'Select the column which contains the <b>Notify parameter</b> of the comment writer (notify if new post paramter)' );
		$rows->createRow('parameter', 
		'--- Website from the column', mosHTML::selectList( $lists['columns'], 'website', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['website'] ), 'Select the column which contains the <b>Website</b> of the comment writer' );
		$rows->createRow('parameter', 
		'--- Title from the column', mosHTML::selectList( $lists['columns'], 'title', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['title'] ), 'Select the column which contains the <b>Title</b> of the comment' );
		$rows->createRow('parameter', 
		'--- Text from the column', mosHTML::selectList( $lists['columns'], 'comment', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['comment'] ), 'Select the column which contains the <b>Text</b> of the comment' );
		$rows->createRow('parameter', 
		'--- Voting Yes count from the column', mosHTML::selectList( $lists['columns'], 'voting_yes', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['voting_yes'] ), 'Select the column which contains the <b>Voting Yes count</b> of the comment' );
        $rows->createRow('parameter', 
		'--- Voting No count from the column', mosHTML::selectList( $lists['columns'], 'voting_no', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['voting_no'] ), 'Select the column which contains the <b>Voting No count</b> of the comment' );
        $rows->createRow('parameter', 
		'--- Published from the column', mosHTML::selectList( $lists['columns'], 'published', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['published'] ), 'Select the column which contains the <b>Published parameter</b> of the comment' );
        $rows->createRow('parameter', 
		'--- Parent id from the column', mosHTML::selectList( $lists['columns'], 'parentid', ' class="inputbox" ', 'Field', 'desc', $lists['sel_columns']['parentid'] ), 'Select the column which contains the <b>Parent Id</b> of the comment (when comment is linked as a child -- response -- of another comment)' );
		$rows->createRow('separator');
		$rows->createRow('parameter', 
