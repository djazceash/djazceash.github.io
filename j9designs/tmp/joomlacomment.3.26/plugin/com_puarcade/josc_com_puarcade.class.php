...
//	var _specific_data;
    ...
    {	
    	...
    	$arcade = mosGetParam( $_REQUEST, "arcade");
    	$gid = mosGetParam( $_REQUEST, "gid", 0);
//    	$fid = mosGetParam( $_REQUEST, "fid","");

    	return  (		$option == 'com_puarcade' 
    				  && 	$arcade == ''
    				  &&  $gid    != 0
    			);	
    }
    ...
    function cleanComponentCache() {
            mosCache::cleanCache( 'com_puarcade' );
    }
...
//				. "\n AND access >= 0"    
	...
	function getObjectIdOption($id=0, $select=true)
	{
		...
		if (!$id && $select && count($content)>0) { 
			array_unshift( $content, mosHTML::makeOption( '0', '-- Select Document --', 'id', 'title' ) );
		}
